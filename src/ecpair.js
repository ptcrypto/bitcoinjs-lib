...
ECPair.makeRandom = function (options) {
  ...
  do {
    d = rng(32)
    typeforce(types.Buffer256bit, d)
  } while (!secp256k1.verifyInt(d))

  return new ECPair(d, null, options)
}

ECPair.prototype.getAddress = function () {
  return baddress.toBase58Check(bcrypto.hash160(this.getPublic()), this.network.pubKeyHash)
}
...
ECPair.prototype.getPublic = function () {
  if (!this.__Q) {
    this.__Q = secp256k1.pointDerive(this.getPrivate(), this.__compressed)
  }
...
ECPair.prototype.toWIF = function () {
  if (!this.__d) throw new Error('Missing private key')
  return wif.encode(this.network.wif, this.getPrivate(), this.__compressed)
}
